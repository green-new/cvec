...
VKH_CreateGraphicsPipeline(
  ...
  vrtx_in_ci.vertexBindingDescriptionCount 
    = pipeline->vert_input_bind_desc_count;
  vrtx_in_ci.pVertexBindingDescriptions = pipeline->vert_input_bind_desc;
  vrtx_in_ci.vertexAttributeDescriptionCount 
    = pipeline->vert_input_attrib_desc_count;
...
VKH_RecordCommandBuffer(
  ...
  {  
    ...
    // vkCmdDraw(buffer, 3, 1, 0, 0); // without index buffer
...
VKH_FindMemoryType(
  ...
  for (Uint32 i = 0; i < gpu_properties.memoryTypeCount; i++) {
    ...
    Uint32 props_match = 
      (gpu_properties.memoryTypes[i].propertyFlags & properties) 
        == properties;
...
VKH_CreateBuffer(
  ...
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to create buffer.");
  }
  ...
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to allocate buffer memory.");
  }
...
VKH_CreateVertexBuffer(
  ...
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to create staging vertex buffer.");
    return res;
  }

  /* Map memory */
  void* data;
  res = vkMapMemory(device, staging_memory, 0, size, 0, &data);
    if (res != VK_SUCCESS) {
      G_Log("ERROR", "Failed to map staging memory to device.");
      return res;
    }
  ...
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to copy staging buffer to vertex buffer.");
    return res;
  }
...
VKH_CreateIndexBuffer(
  ...
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to create staging index buffer.");
    return res;
  }
  ...
  res = vkMapMemory(device, staging_memory, 0, size, 0, &data);
    if (res != VK_SUCCESS) {
      G_Log("ERROR", "Failed to map staging index buffer memory to device.");
      return res;
    }
  ...
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to create index buffer.");
    return res;
  }
  ...
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to copy staging buffer to index buffer.");
    return res;
  }
...
VKH_CopyBuffer(
  ...
  res = vkAllocateCommandBuffers(device, &alloc_info, &cmd_buffer);
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to allocate command buffer for copying buffer.");
    return res;
  }
  ...
  res = vkBeginCommandBuffer(cmd_buffer, &begin_info);
    if (res != VK_SUCCESS) {
      G_Log("ERROR", "Failed to begin command buffer for copying buffer.");
      return res;
    }

    VkBufferCopy copyRegion = { 0 };
    copyRegion.size = size;
    vkCmdCopyBuffer(cmd_buffer, src, dest, 1, &copyRegion);

  res = vkEndCommandBuffer(cmd_buffer);
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to end command buffer for copying buffer.");
    return res;
  }
  ...
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to submit copy buffer command to graphics queue.");
    return res;
  }
  res = vkQueueWaitIdle(graphics_queue);
  if (res != VK_SUCCESS) {
    G_Log("ERROR", "Failed to signal wait on graphics queue.");
    return res;
  }
...
VKH_CreateDescriptorSetLayout(
  ...
  VkResult res = VK_SUCCESS;
  VkDescriptorSetLayoutBinding ubo = { 0 };
  ubo.binding = 0;
  ubo.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
  ubo.descriptorCount = 1;
  ubo.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
  ubo.pImmutableSamplers = NULL;
